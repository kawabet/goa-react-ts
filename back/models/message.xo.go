// Package models contains the types for schema 'dev'.
package models
...
// Message represents a row from 'dev.messages'.
type Message struct {
	ID        int       `json:"id"`         // id
...
func (m *Message) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO dev.messages (` +
...
func (m *Message) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE dev.messages SET ` +
		`room_id = ?, account_id = ?, body = ?, postDate = ?` +
		` WHERE id = ?`
...
func (m *Message) Delete(db XODB) error {
	...
	// sql query
	const sqlstr = `DELETE FROM dev.messages WHERE id = ?`
...
// MessageByID retrieves a row from 'dev.messages' as a Message.
//
...
// MessagesByRoomID retrieves a row from 'dev.messages' as a Message.
//
