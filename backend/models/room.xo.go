...
// Room represents a row from 'dev.rooms'.
type Room struct {
	ID          int       `json:"id"`          // id
...
func (r *Room) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO dev.rooms (` +
...
func (r *Room) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE dev.rooms SET ` +
		`name = ?, description = ?, created = ?` +
		` WHERE id = ?`
...
func (r *Room) Delete(db XODB) error {
	...
	// sql query
	const sqlstr = `DELETE FROM dev.rooms WHERE id = ?`
...
// RoomByName retrieves a row from 'dev.rooms' as a Room.
//
...
// RoomByID retrieves a row from 'dev.rooms' as a Room.
//
